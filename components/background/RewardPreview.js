...
const RewardPreview = ({
    ...
    const initiateTwitterAuth = () => {
        ...
        .then(res => {
            ...
            window.open(`https://api.twitter.com/oauth/authorize?oauth_token=${auth_info.oauth_token}`, '_blank');
            ...
            .then(monitor_res => {
                ...
                setTweetCreds({
                    ...
                    oauth_token_scret: monitor_res.data.data.monitorTwitterAuth.oauth_token_secret
                })
            })
            .catch(monitor_err => {
    ...
    return (<div className="reward-preview-area">
        ...
        />
        { showMedals () }
        <div className="twitter-button" onClick={() => {
            if (tweetCreds == null) {
                initiateTwitterAuth ()
            }
        }}>
            <div className="icon-area"><img src={'https://svgur.com/i/M7e.svg'} width={'80%'} height={'80%'} /></div>
        ...
        {tweetCreds != null && !tweetSent &&
            ...
            <div className="send-tweet-button" onClick={() => { sendTweet () }}>Tell the World!</div>
        </div>
        }
        {tweetSent && <motion.div animate={successTweetControl} className="tweet-sent-message">Tweet made successfully!</motion.div>}
    </div>)
